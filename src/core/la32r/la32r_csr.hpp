...
class la32r_csr {
public:
    la32r_csr(uint32_t core_id, uint32_t &pc, la32r_mmu<nr_tlb_entry> &mmu)
        : core_id(core_id)
        , pc(pc)
        , mmu(mmu) {
        reset();
    ...
    void write(uint32_t index, uint32_t value) {
        ...
        case DMW1: {
            ...
            mmu.dmw_fill(la32r_dmw{new_val->plv0, 0, 0, new_val->plv3, new_val->mat, new_val->pseg, new_val->vseg}, (index == DMW0) ? 0 : 1);
            break;
        }
        default:
            break;
    ...
    void raise_trap(la32r_exccode exc, uint32_t bad_va = 0) {
        ...
        if (exc.first == TLBR || exc.first == ADE || exc.first == ALE || exc.first == PIL || exc.first == PIS || exc.first == PIF || exc.first == PME || exc.first == PPI) {
            badv = bad_va;
        }
        if (exc.first == TLBR || exc.first == PIL || exc.first == PIS || exc.first == PIF || exc.first == PME || exc.first == PPI) {
    ...
    void tlb_write(bool rand) {
        ...
        auto tlbidx_reg  = (csr_tlbidx *)&tlbidx;
        auto estat_reg   = (csr_estat *)&estat;
        mmu.tlb_wr(la32r_tlb{!tlbidx_reg->ne || estat_reg->ecode == 0x3f, asid_reg->asid, tlbelo0_reg->g && tlbelo1_reg->g, tlbidx_reg->ps, tlbehi_reg->vppn, tlbelo0_reg->v, tlbelo0_reg->d, tlbelo0_reg->mat, tlbelo0_reg->plv, tlbelo0_reg->ppn, tlbelo1_reg->v, tlbelo1_reg->d, tlbelo1_reg->mat, tlbelo1_reg->plv, tlbelo1_reg->ppn}, rand ? random : tlbidx_reg->index);
    }
    ...
    void tlb_invalidate(uint8_t op, uint32_t inv_asid, uint32_t inv_va) {
        ...
        case 0x5:
            mmu.tlb_inv([=](const la32r_tlb &tlbe) {
                return tlbe.g == 0 && tlbe.asid == inv_asid && (tlbe.vppn >> (tlbe.ps - 12) == inv_va >> (tlbe.ps + 1));
            });
            break;
        case 0x6:
            mmu.tlb_inv([=](const la32r_tlb &tlbe) {
                return (tlbe.g == 1 || tlbe.asid == inv_asid) && (tlbe.vppn >> (tlbe.ps - 12) == inv_va >> (tlbe.ps + 1));
            });
            break;
        default:
