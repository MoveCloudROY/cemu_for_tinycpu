...
private:
    void exec(uint8_t exc_int) {
        ...
        if_exc = mmu.va_if(pc, (uint8_t *)&instr, csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
        if (if_exc.first != OK) {
        ...
        default:
            ...
            default:
                ...
                case LL_W: {
                    ...
                    la32r_exccode exc = mmu.va_read(va, 4, (uint8_t *)&temp, csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
                    if (exc.first != OK) {
                ...
                case SC_W: {
                    uint32_t va = GPR[instr._2ri14.rj] + (instr._2ri14.i14 << 2);
                    if (csr.get_llbit()) {
                        la32r_exccode exc = mmu.va_write(va, 4, (uint8_t *)&GPR[instr._2ri14.rd], csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
                        if (exc.first != OK) {
                            csr.raise_trap(exc, va);
                        } else {
                            csr.clr_llbit();
                ...
                default:
                    ...
                    case SLTI:
                        ...
                        break;
                    case SLTUI:
                        set_GPR(instr._2ri12.rd, static_cast<uint32_t>(GPR[instr._2ri12.rj]) < static_cast<uint32_t>(instr._2ri12.i12));
                        break;
                    case ADDI_W:
                        set_GPR(instr._2ri12.rd, GPR[instr._2ri12.rj] + instr._2ri12.i12);
                        break;
                    case ANDI:
                        set_GPR(instr._2ri12.rd, static_cast<uint32_t>(GPR[instr._2ri12.rj]) & (static_cast<uint32_t>(instr._2ri12.i12) & 0xfff));
                        break;
                    case ORI:
                        set_GPR(instr._2ri12.rd, static_cast<uint32_t>(GPR[instr._2ri12.rj]) | (static_cast<uint32_t>(instr._2ri12.i12) & 0xfff));
                        break;
                    ...
                    case LD_B: {
                        ...
                        int8_t        temp;
                        la32r_exccode exc = mmu.va_read(va, 1, (uint8_t *)&temp, csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
                        if (exc.first != OK) {
                            csr.raise_trap(exc, va);
                        } else {
                    ...
                    case LD_H: {
                        uint32_t      va = GPR[instr._2ri12.rj] + instr._2ri12.i12;
                        int16_t       temp;
                        la32r_exccode exc = mmu.va_read(va, 2, (uint8_t *)&temp, csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
                        if (exc.first != OK) {
                            csr.raise_trap(exc, va);
                        } else {
                            set_GPR(instr._2ri12.rd, static_cast<int32_t>(temp));
                    ...
                    case ST_B: {
                        uint32_t      va  = GPR[instr._2ri12.rj] + instr._2ri12.i12;
                        la32r_exccode exc = mmu.va_write(va, 1, (uint8_t *)&GPR[instr._2ri12.rd], csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
                        if (exc.first != OK) {
                            csr.raise_trap(exc, va);
                        }
                        break;
                    ...
                    case ST_H: {
                        uint32_t      va  = GPR[instr._2ri12.rj] + instr._2ri12.i12;
                        la32r_exccode exc = mmu.va_write(va, 2, (uint8_t *)&GPR[instr._2ri12.rd], csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
                        if (exc.first != OK) {
                            csr.raise_trap(exc, va);
                        }
                        break;
                    ...
                    case ST_W: {
                        uint32_t      va  = GPR[instr._2ri12.rj] + instr._2ri12.i12;
                        la32r_exccode exc = mmu.va_write(va, 4, (uint8_t *)&GPR[instr._2ri12.rd], csr.get_cur_plv(), csr.get_crmd_pg(), csr.get_asid());
                        if (exc.first != OK) {
                            csr.raise_trap(exc, va);
                        }
                        break;
                    ...
                    default:
                        ...
                        case SLT:
                            ...
                            break;
                        case SLTU:
                            set_GPR(instr._3r.rd, static_cast<uint32_t>(GPR[instr._3r.rj]) < static_cast<uint32_t>(GPR[instr._3r.rk]));
                            break;
                        case NOR:
                            set_GPR(instr._3r.rd, ~(GPR[instr._3r.rj] | GPR[instr._3r.rk]));
                            break;
                        ...
                        case SLL_W:
                            ...
                            break;
                        case SRL_W:
                            set_GPR(instr._3r.rd, static_cast<uint32_t>(GPR[instr._3r.rj]) >> (GPR[instr._3r.rk] & 0x1f));
                            break;
                        case SRA_W:
                            set_GPR(instr._3r.rd, GPR[instr._3r.rj] >> (GPR[instr._3r.rk] & 0x1f));
                            break;
